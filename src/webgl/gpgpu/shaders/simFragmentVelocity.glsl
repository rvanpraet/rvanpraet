...
void main() {
  ...
  // Curl noise
	...
	vec3 transitionNoise = curlNoise( vec3(position.x * 1., position.y * 10., position.z * 5.) ) * 0.005;
  ...
  velocity.x += sin(uTime * 1.2) * dist * 0.005; // Adds extra wiggle
	velocity += (direction * ( dist * 0.01 ) + transitionNoise * dist * 1.0) * step(0.001, dist); // Force that pushes particles towards their current target
